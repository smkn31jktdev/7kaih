...
import { MoveRight, Search, Filter, Moon } from "lucide-react";
import type { TidurStudent } from "./types";

interface TidurStudentListProps {
  students: TidurStudent[];
  searchQuery: string;
  onSearchChange: (query: string) => void;
  onSelectStudent: (student: TidurStudent) => void;
}

export default function TidurStudentList({
  students,
  searchQuery,
  onSearchChange,
  onSelectStudent,
}: TidurStudentListProps) {
        ...
        <div className="flex flex-col md:flex-row items-center gap-5 text-center md:text-left">
          ...
          <div>
            ...
            <p className="text-gray-500">
              Rekapitulasi aktivitas tidur malam siswa.
            </p>
          </div>
        </div>
      </div>
