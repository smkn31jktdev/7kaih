...
import { Check, User, X, ArrowDownToLine } from "lucide-react";
import Select from "@/app/components/Select";
import type { BangunStudent, BangunEntry } from "./types";
import { formatDisplayDate, formatMonthLabel } from "../utils";

interface BangunDetailModalProps {
  student: BangunStudent;
  filteredEntries: BangunEntry[];
...
export default function BangunDetailModal({
  student,
  filteredEntries,
  selectedMonth,
  availableMonths,
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 text-gray-500 font-semibold border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-6 py-4 text-center">Jam Bangun</th>
                      ...
                      <td className="px-6 py-4 text-center">
                        {entry.berdoa ? (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold">
                            <Check className="w-3.5 h-3.5" /> Berdoa
                          </span>
                        ) : (
