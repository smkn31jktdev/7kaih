...
import { MoveRight, Search, Filter, Sun } from "lucide-react";
import type { BangunStudent } from "./types";

interface BangunStudentListProps {
  students: BangunStudent[];
  searchQuery: string;
  onSearchChange: (query: string) => void;
  onSelectStudent: (student: BangunStudent) => void;
}

export default function BangunStudentList({
  students,
  searchQuery,
  onSearchChange,
  onSelectStudent,
}: BangunStudentListProps) {
        ...
        <div className="flex flex-col md:flex-row items-center gap-5 text-center md:text-left">
          ...
          <div>
            ...
            <p className="text-gray-500">
              Rekapitulasi aktivitas bangun pagi siswa.
            </p>
          </div>
        </div>
      </div>
