...
export default function AdminTidurPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    const renderBaseLayout = () => {
      ...
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(16);
      doc.text("Tidur Cepat", pageCenterX, 45, { align: "center" });
    ...
    const renderTableHeader = () => {
      ...
      doc.line(
        startX + dateColWidth + sleepColWidth,
        startY,
        startX + dateColWidth + sleepColWidth,
        startY + headerMainHeight + headerSubHeight
      );
      doc.line(
        startX + dateColWidth + sleepColWidth,
        startY + headerMainHeight,
        startX + dateColWidth + sleepColWidth + prayerColWidth,
        startY + headerMainHeight
      );
      doc.line(
        startX + dateColWidth + sleepColWidth + prayerColWidth / 2,
        startY + headerMainHeight,
        startX + dateColWidth + sleepColWidth + prayerColWidth / 2,
        startY + headerMainHeight + headerSubHeight
      );
      ...
      doc.text(
        "Aku Tidur Jam?",
        startX + dateColWidth + sleepColWidth / 2,
        combinedHeaderCenter,
        { align: "center" }
      );

      const prayerHeader = "Membaca doa sebelum tidur?";
      ...
      doc.text(
        "YA",
        startX + dateColWidth + sleepColWidth + prayerColWidth / 4,
        subHeaderCenter,
        { align: "center" }
      );
      doc.text(
        "TIDAK",
        startX + dateColWidth + sleepColWidth + (3 * prayerColWidth) / 4,
        subHeaderCenter,
        { align: "center" }
      );
    ...
    for (let index = 0; index < rowsToRender; index += 1) {
      ...
      doc.rect(marginX + dateColWidth, rowTop, sleepColWidth, rowHeight);
      doc.rect(
        marginX + dateColWidth + sleepColWidth,
        rowTop,
        prayerColWidth / 2,
        rowHeight
      );
      doc.rect(
        marginX + dateColWidth + sleepColWidth + prayerColWidth / 2,
        rowTop,
        prayerColWidth / 2,
        rowHeight
      );
      ...
      if (entry) {
        ...
        doc.text(
          entry.jamTidur || "-",
          marginX + dateColWidth + sleepColWidth / 2,
          rowTop + rowHeight / 2 + 1,
          {
            align: "center",
          }
        ...
        const yaCenterX =
          marginX + dateColWidth + sleepColWidth + prayerColWidth / 4;
        const tidakCenterX =
          marginX + dateColWidth + sleepColWidth + (3 * prayerColWidth) / 4;
                ...
                <div className="flex items-center justify-between">
                  <div className="w-full text-center">
                    <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-1 sm:mb-2 md:mb-3 flex items-center justify-center gap-2 sm:gap-3 md:gap-4">
                      <Moon className="w-6 h-6 sm:w-8 sm:h-8 text-white/90" />
                      <span>Data Tidur Cepat Siswa</span>
                    </h1>
                    <p className="text-blue-100 text-xs sm:text-sm md:text-base lg:text-lg max-w-3xl mx-auto">
                      Pantau aktivitas tidur cepat siswa dan status berdoa
                              ...
                              <div className="h-64 md:h-72 lg:h-[18rem] overflow-y-auto overscroll-contain scrollbar-hide">
                                <div className="space-y-3">
                                  {students.map((student) => (
                                    <div
                                      key={student.nisn}
                                      className="flex items-center justify-between gap-4 rounded-2xl bg-white/80 border border-indigo-50 px-4 py-4 hover:bg-indigo-50/50 transition-colors"
                                    >
                                      <div className="flex items-center gap-4 min-w-0">
                                        <div className="w-12 h-12 rounded-xl bg-indigo-100 flex items-center justify-center shrink-0">
                                      ...
                                      <button
                                        ...
                                        className="flex items-center gap-2 rounded-full bg-indigo-500 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-indigo-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2"
                                      >
                                        Detail
