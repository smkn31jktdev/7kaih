...
export async function GET(request: NextRequest) {
  try {
    ...
    const grouped = new Map<string, BermasyarakatStudent>();
    ...
    documents.forEach((doc) => {
      ...
      if (student && kebiasaan.bermasyarakat) {
        student.entries.push({
          tanggal: kebiasaan.tanggal,
          deskripsi: kebiasaan.bermasyarakat.deskripsi || "",
          tempat: kebiasaan.bermasyarakat.tempat || "",
          waktu: kebiasaan.bermasyarakat.waktu || "",
          paraf: kebiasaan.bermasyarakat.paraf || false,
        });
      }
    });

    const data: BermasyarakatStudent[] = Array.from(grouped.values()).map(
      (student) => ({
        ...student,
    ...
    console.error("Error fetching bermasyarakat data:", error);
    return NextResponse.json(
